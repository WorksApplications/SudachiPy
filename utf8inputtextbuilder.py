...
class UTF8InputTextBuilder(inputtextbuilder.InputTextBuilder):
    def __init__(self, text, grammar):
        ...
        # 注: サロゲートペア文字は考慮していない
        self.text_offsets = [i for i in len(self.original_text)]
    ...
    def build(self):
        ...
        for i in range(len(self.modified_text)):
            # 注: サロゲートペア文字は考慮していない
            for j in range(self.utf8_byte_length(self.modified_text.encode('utf-8'))):
                byte_indexes[i] = i
                offsets[i] = self.text_offsets[i]
    ...
    def get_char_category_types(self, text):
        ...
        for i in range(len(text)):
            types = self.grammar.get_character_category().get_category_types(text.encode('utf-8'))
    ...
    def get_char_category_continuities(self, text, byte_length, char_categories):
        ...
        while i < len(char_categories):
            ...
            for j in range(i, next):
                length += self.utf8_byte_length(text.encode('utf-8'))
    ...
    def get_char_category_continuous_length(self, char_categories, offset):
        ...
        for length in range(1, len(char_categories) - offset):
            continuous_category.retain_all(char_categories[offset + length])
